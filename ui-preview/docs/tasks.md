...
**Checkpoint**: All four user stories independently functional — Phase 1 validation target ("register, sign in with MFA, and access a role-scoped, localized dashboard within an isolated tenant") is met
...
- **User Story 2 (Phase 4)**: Depends on Foundational; T043 also depends on US1's routes existing (T022–T029) to decorate
- **User Story 3 (Phase 5)**: Depends on Foundational; independently testable once T053 wires the shared guard, but T054's RLS policies apply to tables introduced across Foundational + US1 + US2
- **User Story 4 (Phase 6)**: Depends on Foundational; its implementation tasks (T059, T062) apply localization on top of the screens built in US1–US3, so it is sequenced last even though the i18n scaffold itself (T015, T016) is Foundational
