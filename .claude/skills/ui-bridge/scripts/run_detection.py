...
def layer_1(root: Path) -> dict:
    ...
    if tasks_path:
        ...
        tasks_found = len(re.findall(r'^- \[[ xX]\]\s+T\d+', content, re.MULTILINE))
        # Some Spec Kit templates keep Phase 0/1 as separate research.md/data-model.md
        # deliverables and put the real phase breakdown in tasks.md instead of plan.md.
        tasks_phases_found = len(re.findall(r'^#{2,3}\s+Phase', content, re.MULTILINE))
        if tasks_found == 0 and status == 'ok':
            status = 'empty'
    else:
        status = 'missing_tasks'

    if status == 'ok' and phases_found == 0 and tasks_phases_found == 0:
        status = 'empty'
    ...
    return {
        ...
        'tasks_phases_found': tasks_phases_found,
...
def layer_4(root: Path, l1: dict) -> dict:
    ...
    task_lines = re.findall(r'^- \[[ xX]\]\s+(T\d+.*)$', tasks_content, re.MULTILINE)
